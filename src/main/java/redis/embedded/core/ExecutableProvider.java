...
import static redis.embedded.model.OsArchitecture.UNIX_AARCH64_MUSL;
import static redis.embedded.model.OsArchitecture.UNIX_x86;
import static redis.embedded.model.OsArchitecture.UNIX_x86_64;
import static redis.embedded.model.OsArchitecture.UNIX_x86_64_MUSL;
import static redis.embedded.model.OsArchitecture.WINDOWS_x86_64;
import static redis.embedded.model.OsArchitecture.detectOSandArchitecture;
...
public interface ExecutableProvider {
    Logger LOG = Logger.getLogger(ExecutableProvider.class.getName());
    ...
    URI
            ...
            REDIS_7_2_5_WINDOWS_ZKTECO = URI.create("https://github.com/codemonstur/embedded-redis/raw/master/src/main/binaries/redis-server-7.2.5-windows"), REDIS_7_2_MACOSX_14_SONOMA_HANKCP = URI.create("https://github.com/codemonstur/embedded-redis/raw/master/src/main/binaries/redis-server-7.2-darwin-arm64"), // Downloaded from: https://packages.redis.io/redis-stack/redis-stack-server-7.2.0-v6.jammy.x86_64.tar.gz
    REDIS_7_2_LINUX_JAMMY_X86_64 = URI.create("https://github.com/codemonstur/embedded-redis/raw/master/src/main/binaries/redis-stack-server-7.2.0-v6.jammy.x86_64"), // Downloaded from: https://packages.redis.io/redis-stack/redis-stack-server-7.2.0-v6.jammy.arm64.tar.gz
    ...
    static ExecutableProvider newJarResourceProvider(final IOSupplier<File> tempDirectory, final Map<OsArchitecture, String> executables) {
        ...
        LOG.info("Detected OS: os=" + osArch.os + " arch=" + osArch.arch + " cLib=" + osArch.cLib);
    ...
    static ExecutableProvider newCachedUrlProvider(final Path cachedLocation, final URI uri) {
        return () -> {
            if (isRegularFile(cachedLocation)) return cachedLocation.toFile();
            ...
            try {
                ...
                try (final OutputStream out = newOutputStream(cachedLocation, CREATE, WRITE, TRUNCATE_EXISTING); final InputStream in = connection.getInputStream()) {
